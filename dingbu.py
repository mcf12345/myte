...
# ---------------------- 核心：顶部选项卡导航 ----------------------
tab_home, tab_video, tab_music, tab_album, tab_ktv, tab_resume = st.tabs([
    "🏠 首页", 
    "🎬 视频中心", 
    "🎵 音乐播放器", 
    "🖼️ 相册网站", 
...
# ---------------------- 首页（顶部选项卡1） ----------------------
with tab_home:
    st.header("欢迎来到综合多媒体网站（顶部栏版）")
    st.write("这是整合了多个实训内容的完整网站，通过顶部选项卡切换不同功能。")
...
with tab_video:
    ...
    # 会话状态：保存当前播放索引
    if 'video_ind_tab' not in st.session_state:
        st.session_state.video_ind_tab = 0
    
    # 显示当前视频
    st.subheader(video_arr[st.session_state.video_ind_tab]['title'])
    st.video(video_arr[st.session_state.video_ind_tab]['url'])
    ...
    for i, col in enumerate(cols):
        with col:
            st.button(
                f"第{i+1}集",
                key=f"video_btn_tab_{i}",
                on_click=lambda idx=i: st.session_state.update(video_ind_tab=idx),
...
with tab_music:
    ...
    # 会话状态
    if 'music_ind_tab' not in st.session_state:
        st.session_state.music_ind_tab = 0
    
    # 列容器：封面 + 播放区域
    col1, col2 = st.columns([1, 3])
    with col1:
        st.image(music_list[st.session_state.music_ind_tab]['img'], use_container_width=True)
    with col2:
        st.subheader(music_list[st.session_state.music_ind_tab]['title'])
        st.audio(music_list[st.session_state.music_ind_tab]['url'], format="audio/mp3")
    ...
    with col_prev:
        st.button(
            "⬅️ 上一首",
            key="music_prev_tab",
            on_click=lambda: st.session_state.update(music_ind_tab=(st.session_state.music_ind_tab-1)%len(music_list)),
            use_container_width=True
        )
    with col_next:
        st.button(
...
with tab_album:
    ...
    # 会话状态
    if 'image_ind_tab' not in st.session_state:
        st.session_state.image_ind_tab = 0
    
    # 主图展示
    st.image(
        image_list[st.session_state.image_ind_tab]['url'],
        caption=image_list[st.session_state.image_ind_tab]['title'],
        use_container_width=True
    )
    ...
    with col_prev:
        st.button(
            "⬅️ 上一张",
            key="img_prev_tab",
            on_click=lambda: st.session_state.update(image_ind_tab=(st.session_state.image_ind_tab-1)%len(image_list)),
            use_container_width=True
        )
    with col_next:
        st.button(
...
with tab_resume:
    ...
    with col_form:
        st.subheader("填写个人信息")
        # 基础信息
        name = st.text_input("姓名", "罗雨湾", key="resume_name")
        position = st.text_input("应聘职位", "软件测试", key="resume_pos")
        phone = st.text_input("联系电话", "17677169536", key="resume_phone")
        email = st.text_input("邮箱", "237917611@qq.com", key="resume_email")
        education = st.selectbox("学历", ["本科", "硕士", "博士"], index=0, key="resume_edu")
        
        # 扩展器：更多信息
        with st.expander("🔍 更多信息", expanded=False):
            gender = st.radio("性别", ["女", "男"], index=0, key="resume_gender")
            skills = st.multiselect("专业技能", ["Python", "Java", "SQL", "测试"], default=["Python", "测试"], key="resume_skills")
            bio = st.text_area("个人简介", "具备5年软件测试经验，熟悉自动化测试流程。", key="resume_bio")
